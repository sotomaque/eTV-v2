...
export default function SectionPopularDramasOfThisYear() {
  const classes = useStyles();
  const [data, setData] = useState({ hits: [] });
  const [url, setUrl] = useState(
    'https://api.themoviedb.org/3/discover/movie?api_key=c9f3c719e4cce4a021ff37d2e89d43ba&with_genres=18&primary_release_year=2019',
  );
  ...
  return (
    <div className={classes.section}>
      ...
      { isError ? <div>Something went wrong loading sectionPopularDramasOfThisYear...</div> : 

        <div id="images">
          <div className={classes.title}>
            <h2>Popular Dramas of 2019</h2>
          </div>
          <br />
          {
